...
from modals.genesis import gen_prompt_stream, gen_function_call as genesis_function_call
import base64
...
def generate_image(prompt):
    # Styles including quality terms
    dreamscape_styles = [
    'dreamscape', 'anime', 'ghibli', 'high resolution', 'ultra-realistic', 'detailed', 
    'cinematic', 'hyper-detailed', 'ethereal', 'surreal', 'vivid colors', 
    'cinematic lighting', 'concept art', 'textured', '3D render', 'vibrant hues', 
    'epic scale', 'wide-angle view', 'otherworldly'
]
    ...
    try:
        if any(style in prompt_lower for style in dreamscape_styles):
            # Add "in dreamscape style" if not explicitly included
...
def streamlit_ui():
    ...
    # Model selection
    model_options = ["Genesis", "Genesis-o1"]
    selected_model = st.sidebar.selectbox(
        "",
        options=model_options,
        index=0,  # Set "Genesis" as default
    )
            ...
            try:
                ...
                if function_result["function"] == "generate_image":
                    ...
                    if selected_model == "Genesis":
                        ...
                        response = gen_prompt_stream(prompt=prompt)
                        st.write(response)
