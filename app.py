...
def streamlit_ui():
    ...
    # Spinner Styles for Dark and Light Themes
    spinner_style = """
        <style>
        /* For dark theme */
        .st-dark .stSpinner > div {
            border-top-color: #ffffff !important;
            border-left-color: #ffffff !important;
        }

        /* For light theme */
        .st-light .stSpinner > div {
            border-top-color: #FF69B4 !important; /* Barbie Pink */
    ...
    # Strobe effect JavaScript
    strobe_script = """
    <script>
    // Function to add the strobe effect class for 10 seconds
    document.addEventListener('DOMContentLoaded', function() {
        const header = document.querySelector('.header-content');
        header.addEventListener('click', function() {
            header.classList.add('strobe-active');
            
            // Remove the class after 10 seconds
            setTimeout(function() {
                header.classList.remove('strobe-active');
            }, 10000); // 10 seconds
        });
    });
    </script>
    """
    st.markdown(strobe_script, unsafe_allow_html=True)
